...
class SqliteDiskStorage<T>(
    ...
    private val container: FlowTrackingContainer,
    ...
    override fun getAllWith(options: QueryOptions): Flow<PagingList<T>?> {
        return container.create(tableName) { findAllWith(options) }
    }

    override fun getAll(folder: String?): Flow<List<T>?> {
        return container.create(tableName) { findAll(folder) }
    }

    override fun getById(id: String): Flow<T?> {
